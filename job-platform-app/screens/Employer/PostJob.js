...
const PostJob = () => {
    ...
    const [form, setForm] = useState({ job_type: 'full_time' });
    ...
    const submit = async () => {
        ...
        try {
            ...
            await authApis(token).post(endpoints['employer-jobs'], form);
    ...
    const FIELDS = [
        ...
        { key: 'required_skills', label: 'Kỹ năng yêu cầu (phân cách bởi dấu phẩy)', icon: 'tools' },
    ...
    return (
        <ScrollView style={Styles.container} contentContainerStyle={[Styles.padding, { paddingBottom: 40 }]}>
            <Text style={[Styles.subject, { marginBottom: 12 }]}>📝 Đăng tin tuyển dụng</Text>
