...
const MenuItem = ({ icon, title, subtitle, onPress, color }) => (
    <TouchableOpacity style={[Styles.card, { flexDirection: 'row', alignItems: 'center' }]} onPress={onPress} activeOpacity={0.8}>
...
const EmployerDashboard = () => {
    ...
    return (
        <ScrollView style={Styles.container} contentContainerStyle={{ paddingBottom: 30 }}>
            ...
            <MenuItem icon="📝" title="Đăng tin tuyển dụng" subtitle="Tạo tin tuyển dụng mới" color="#E3F2FD" onPress={() => nav.navigate('post-job')} />
            <MenuItem icon="📋" title="Danh sách tin đăng" subtitle="Xem và quản lý tin tuyển dụng" color="#F3E5F5" onPress={() => nav.navigate('manage-jobs')} />

            <Text style={Styles.sectionHeader}>Quản lý ứng viên</Text>
            <MenuItem icon="👥" title="Xem hồ sơ ứng tuyển" subtitle="Duyệt và xem xét hồ sơ ứng viên" color="#E8F5E9" onPress={() => nav.navigate('view-applications')} />
        </ScrollView>
    );
};
